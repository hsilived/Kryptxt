...
// segmented control idea was given by Adam Roberts, Managing Director at Enigmatic Flare, 2012
        ...
        if (_extraButton != NULL && _extraButton != @"") {
...
//NSLog(@"currentFrame - %f,%f,%f,%f", currentFrame.origin.x, currentFrame.origin.y, currentFrame.size.width, currentFrame.size.height);
...
//todo need to figure out how to get this value programmitically
    ...
    NSLog(@"top of text box %d - %d", selectedTextFieldOrView.tag, top);
...
//NSLog(@"offset (%f - %f)- %f", initialFrame.origin.y, currentFrame.origin.y, offset);
//NSLog(@"diff (%d - %f - %f)- %f", top, visibleY, offset, diff);
...
- (void)updateBarHelper {
    ...
    if ((selectedTextFieldOrView.tag != 0) && (selectedTextFieldOrView.tag == 5) && (_extraButton != @"")) {
