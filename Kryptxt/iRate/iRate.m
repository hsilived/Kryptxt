...
- (NSURL *)ratingsURL
{
    ...
    if (!self.appStoreID)
        RLog(@"iRate could not find the App Store ID for this application. If the application is not intended for App Store release then you must specify a custom ratingsURL.");
...
{   
    ...
    if (self.previewMode)
    {
        RLog(@"iRate preview mode is enabled - make sure you disable this for release");
        return YES;
    }
    ...
    else if (self.ratedThisVersion)
    {
        ...
        {
            RLog(@"iRate did not prompt for rating because the user has already rated this version");
        }
        return NO;
    }
    ...
    else if (!self.promptAgainForEachNewVersion && self.ratedAnyVersion)
    {
        if (self.verboseLogging)
        {
            RLog(@"iRate did not prompt for rating because the user has already rated this app, and promptAgainForEachNewVersion is disabled");
        }
        return NO;
    }
    ...
    else if (self.declinedThisVersion)
    {
        if (self.verboseLogging)
        {
            RLog(@"iRate did not prompt for rating because the user has declined to rate this version");
        }
        return NO;
    }
    ...
    else if ((self.daysUntilPrompt > 0.0f || self.usesPerWeekForPrompt) && self.firstUsed == nil)
    {
        if (self.verboseLogging)
        {
            RLog(@"iRate did not prompt for rating because this is the first time the app has been launched");
        }
        return NO;
    }
    ...
    else if ([[NSDate date] timeIntervalSinceDate:self.firstUsed] < self.daysUntilPrompt * SECONDS_IN_A_DAY)
    {
        if (self.verboseLogging)
        {
            RLog(@"iRate did not prompt for rating because the app was first used less than %g days ago", self.daysUntilPrompt);
        }
        return NO;
    }
    ...
    else if (self.usesCount < self.usesUntilPrompt && self.eventCount < self.eventsUntilPrompt)
    {
        if (self.verboseLogging)
        {
            RLog(@"iRate did not prompt for rating because the app has only been used %@ times and only %@ events have been logged", @(self.usesCount), @(self.eventCount));
        }
        return NO;
    }
    ...
    else if (self.usesPerWeek < self.usesPerWeekForPrompt)
    {
        if (self.verboseLogging)
        {
            RLog(@"iRate did not prompt for rating because the app has only been used %g times per week on average since it was installed", self.usesPerWeek);
        }
        return NO;
    }
    ...
    else if (self.lastReminded != nil && [[NSDate date] timeIntervalSinceDate:self.lastReminded] < self.remindPeriod * SECONDS_IN_A_DAY)
    {
        if (self.verboseLogging)
        {
            RLog(@"iRate did not prompt for rating because the user last asked to be reminded less than %g days ago", self.remindPeriod);
        }
        return NO;
    }
    ...
    if (self.checkingForPrompt)
    {
        ...
        if ([self.delegate respondsToSelector:@selector(iRateShouldPromptForRating)])
        {
            if (![self.delegate iRateShouldPromptForRating])
            {
                if (self.verboseLogging)
                {
                    RLog(@"iRate did not display the rating prompt because the iRateShouldPromptForRating delegate method returned NO");
                }
                return;
            }
        }
...
- (void)connectionError:(NSError *)error
{
    ...
    if (error)
    {
        RLog(@"iRate rating process failed because: %@", [error localizedDescription]);
    }
    else
    {
        RLog(@"iRate rating process failed because an unknown error occured");
    }
...
- (void)checkForConnectivityInBackground
{
    @synchronized (self)
    {
        @autoreleasepool
        {
            ...
            if (self.verboseLogging)
            {
                RLog(@"iRate is checking %@ to retrieve the App Store details...", iTunesServiceURL);
            }
            ...
            if (data && statusCode == 200)
            {
                ...
                if (!error)
                {
                    ...
                    if (bundleID)
                    {
                        if ([bundleID isEqualToString:self.applicationBundleID])
                        {
                            ...
                            if (!self.appStoreID)
                            {
                                ...
                                if (self.verboseLogging)
                                {
                                    RLog(@"iRate found the app on iTunes. The App Store ID is %@", appStoreIDString);
                                }
                            }
                            ...
                            if (self.onlyPromptIfLatestVersion && !self.previewMode)
                            {
                                ...
                                if ([latestVersion compare:self.applicationVersion options:NSNumericSearch] == NSOrderedDescending)
                                {
                                    if (self.verboseLogging)
                                    {
                                        RLog(@"iRate found that the installed application version (%@) is not the latest version on the App Store, which is %@",
                        ...
                        else
                        {
                            if (self.verboseLogging)
                            {
                                RLog(@"iRate found that the application bundle ID (%@) does not match the bundle ID of the app found on iTunes (%@) with the specified App Store ID (%@)", self.applicationBundleID, bundleID, @(self.appStoreID));
                            }
                    ...
                    else if (self.appStoreID || !self.ratingsURL)
                    {
                        if (self.verboseLogging)
                        {
                            RLog(@"iRate could not find this application on iTunes. If your app is not intended for App Store release then you must specify a custom ratingsURL. If this is the first release of your application then it's not a problem that it cannot be found on the store yet");
                        }
                    ...
                    {
                        RLog(@"iRate could not find your app on iTunes. If your app is not yet on the store or is not intended for App Store release then don't worry about this");
                    }
                }
            }
...
- (BOOL)openRatingsPageInAppStore
{
    if (!_ratingsURL && !self.appStoreID)
    {
        if (self.verboseLogging)
        {
            RLog(@"iRate was unable to open the App Store because the app store ID is not set.");
        }
        return NO;
    }
    ...
    if (!_ratingsURL && [SKStoreProductViewController class])
    {
        if (self.verboseLogging)
        {
            RLog(@"iRate will attempt to open the StoreKit in-app product page using the following app store ID: %@", @(self.appStoreID));
        }
        ...
        if (!rootViewController)
        {
            if (self.verboseLogging)
            {
                RLog(@"iRate couldn't find root view controller from which to display StoreKit product page");
            }
        }
        else
        {
    ...
    if (self.verboseLogging)
    {
        RLog(@"iRate will open the App Store ratings page using the following URL: %@", self.ratingsURL);
    }
